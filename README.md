...
## Запуск
<a name="usage"></a>
Для запуска тестов через GoogleChrome выполнить следующие команда:
```
mvn clean test -Dbrowser=chrome allure:serve
```
Для запуска тестов через Yandex Browser выполнить следующие команда:
```
...
## Классы
<a name="classes"></a>
* `Endpoints` - эндпоинты для создания, авторизации и удаления пользователя;
* `HomePage` - POM главной страницы;
* `LoginPage` - POM страницы авторизации пользователя;
* `RecoveryPasswordPage` - POM страницы восстановления пароля;
* `RegisterPage` - POM страницы регистрации пользователя;
* `UserPage` - POM страницы личного кабинета пользователя;
...
## Тесты
<a name="tests"></a>
* `ConstructorTest` - проверка переходов по вкладкам в конструкторе;
* `PersonalAccountTest` - проверка личного кабинета;
* `PersonalConstructorTest` - проверка перехода в конструктор из личного кабинета;
* `UserLoginTest` - проверка авторизации пользователя;
* `UserRegistrationTest` - проверка регистрации пользователя с корректными и некорректными данными;
